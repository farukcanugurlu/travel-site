...
const FeatureSidebar = ({ setProducts }: FeatureSidebarProps) => {
  ...
  const filterdProduct = allProducts.filter(
    (product) => product.page === "shop_3"
  );
  ...
  const handleDuration = (duration: string) => {
    setDurationSelected((prevDuration) =>
      prevDuration === duration ? "" : duration
    );
    filterProducts({
      duration: duration === durationSelected ? "" : duration,
  ...
  const handleAmenities = (amenities: string) => {
    setAmenitiesSelected((prevAmenities) =>
      prevAmenities === amenities ? "" : amenities
    );
    filterProducts({
      amenities: amenities === amenitiesSelected ? "" : amenities,
      language: languageSelected,
      rating: ratingSelected,
  ...
  const handleLanguage = (language: string) => {
    ...
    filterProducts({
      amenities: amenitiesSelected,
      language: language === languageSelected ? "" : language,
      rating: ratingSelected,
      duration: durationSelected,
    });
  };
  ...
  const filterProducts = ({
    ...
    if (amenities && amenities !== "All Amenities") {
      filteredProducts = filteredProducts.filter(
        (product) => product.amenities === amenities
      );
    }
                ...
                <li key={i} onClick={() => handleDuration(duration)}>
                  <div className="checkbox d-flex">
                    <input
                      ...
                      type="checkbox"
                      checked={duration === durationSelected}
                      readOnly
                      id={`duration_${i}`}
                    />
                    <label
                ...
                <li key={i} onClick={() => handleAmenities(amenities)}>
                  <div className="checkbox d-flex">
                    <input
                      className="tg-checkbox"
                      type="checkbox"
                      checked={amenities === amenitiesSelected}
                      readOnly
                      id={`amenities_${i}`}
                    />
                    <label
                      ...
                      onClick={() => handleAmenities(amenities)}
                    >
                      {amenities}
                    </label>
                  </div>
                ...
                <li key={i} onClick={() => handleLanguage(language)}>
                  <div className="checkbox d-flex">
                    <input
                      className="tg-checkbox"
                      type="checkbox"
                      checked={language === languageSelected}
                      readOnly
                      id={`language_${i}`}
                    />
                    <label
