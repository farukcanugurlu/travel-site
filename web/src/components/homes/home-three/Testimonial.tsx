...
const Testimonial = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, index) => (
      <span key={index}>
        <i 
          className={`fa-sharp fa-solid fa-star ${index < rating ? '' : 'text-muted'}`}
  ...
  const getAvatarUrl = (name: string) => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=560CE3&color=fff&size=60&bold=true`;
  };
  return (
                ...
                return (
                  <SwiperSlide key={review.id} className="swiper-slide">
                    <div className="tg-testimonial-item mb-30">
                      ...
                      <p className="tg-testimonial-avatar-para mb-10">
                        {review.content}
                      </p>
